...
def register():
    ...
    form = RegisterForm()
    if form.validate_on_submit(): #form düzgün doldurulmuşsa.
...
def login():
    ...
    if form.validate_on_submit():
        ...
        if user and user.password == form.password.data:
            ...
            flash(f'Giriş başarısız', 'danger')
...
def new_post():
    ...
    if form.validate_on_submit():
        ...
        db.session.add(post)
        db.session.commit()
        flash(f'Post oluşumu başarılı', 'success')
        return redirect(url_for('index'))
    return render_template('create_post.html',title='Post oluştur',form=form)
...
def edit_post(post_id):
    ...
    if form.validate_on_submit():
        ...
        db.session.commit()
        flash(f'Post düzenlendi', 'success')
...
def contact():
    ...
    if request.method == 'POST':
        if form.validate_on_submit():
            flash(f'Mesaj iletildi', 'success')
            return redirect(url_for('contact'))
        else:
            flash(f'Bir sorun var', 'danger')
    elif request.method == 'GET':
